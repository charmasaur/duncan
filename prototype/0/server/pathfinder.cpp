 ...
 */
PathFinder::PathFinder(const PathWorld* world) : world_(world) {
 ...
 */
bool PathFinder::path(size_t r1,size_t c1,size_t r2,size_t c2,Character* ignore,path_t& p) {
	if(world_==0) return false;
	...
	while(!queue_.empty()) {
		...
		for(size_t d=0; d<4; d++) {
			...
			if(map_.at((ne=encode(nr,nc))).first) continue;
			map_.at(ne).first=true;
			if(!world_->is_vacant(nr,nc,ignore)) continue;
			map_.at(ne).second=cur;
			queue_.push(ne);
		}
	}
	return false;
